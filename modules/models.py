...
class AETN(object):
    ...
    def parse_params(self, params):
        ...
        if params['q_dims'] is None:
            ...
            assert params['q_dims'][0] == params['p_dims'][
    ...
    def forward_pass(self):
        ...
        new_bert_masks = tf.tile(tf.expand_dims(self.newbertmask * self.training * self.train_mlm, 2),
                                 [1, 1, self.dae_dims[1]])  # (N, length, d_model)
        loss_bert_masks = tf.tile(tf.expand_dims(self.lossbertmask * self.training * self.train_mlm, 2),
                                  [1, 1, self.dae_dims[1]])  # (N, length, d_model)
        ...
        img_emb = img_emb + tf.nn.embedding_lookup(self.img_embeddings,
                                                   tf.zeros_like(tf.reduce_sum(img_emb, 2),
                                                                 tf.int32))  # (N ,1, d_model)
        ...
        for trans_layer in range(self.num_translayer):
            ...
            img_new_emb = tf.concat([tf.tile(img_emb, [1, tf.shape(new_emb)[1], 1]), new_emb],
                                    2)  # (N, length_his, 2*d_model)
            img_loss_emb = tf.concat([tf.tile(img_emb, [1, tf.shape(loss_emb)[1], 1]), loss_emb],
                                     2)  # (N, length_his, 2*d_model)
            img_his_emb = self.ffn(self.multihead_attention(queries=tf.concat([img_emb, new_emb, loss_emb], 1),
                                                            ...
                                                            query_masks=tf.concat(
                                                                [imgmask, self.newmask, self.lossmask], 1),
                                                            key_masks=tf.concat([self.newmask, self.lossmask],
                                                                                1)))  # (N, 2*length_his+1, 512)
        ...
        user_embeddings = tf.matmul(tf.squeeze(img_emb, [1]), self.bottleneck_weights[0]) + self.bottleneck_biases[
            0]  # (N, 128)
        ...
        logits_bert_new = tf.einsum('ijk,kl->ijl', tf.batch_gather(new_emb, self.newbertindex), self.dae_weights[-1]) + \
                          self.dae_biases[-1]
        model_outputs.append(logits_bert_new)
        logits_bert_loss = tf.einsum('ijk,kl->ijl', tf.batch_gather(loss_emb, self.lossbertindex),
                                     self.dae_weights[-1]) + self.dae_biases[-1]
        ...
        img_new_emb = tf.concat([tf.tile(use_emb, [1, tf.shape(new_emb)[1], 1]), new_pos_emb],
                                2)  # (N ,length_his, 128+d_model)
        img_new_emb = reduce_dim_layer(img_new_emb)  # (N ,length_his, d_model)

        img_loss_emb = tf.concat([tf.tile(use_emb, [1, tf.shape(loss_emb)[1], 1]), loss_pos_emb],
                                 2)  # (N, length_his, 128+d_model)
        ...
        his_emb = self.ffn(self.multihead_attention(queries=tf.concat([img_new_emb, img_loss_emb], 1),
                                                    ...
                                                    key_masks=tf.concat([self.newmask, self.lossmask], 1)))  # (N,
        # 2*length_his, 512)
        # Main reconstruction of the installation and uninstallation
        logits_new = tf.einsum('ijk,kl->ijl', his_emb[:, :self.length_his, :], self.dae_weights[-1]) + self.dae_biases[
            -1]  # (N, length_his, num_softid)
        model_outputs.append(logits_new)
        logits_loss = tf.einsum('ijk,kl->ijl', his_emb[:, -self.length_his:, :], self.dae_weights[-1]) + \
                      self.dae_biases[-1]  # (N, length_his, num_softid)
    ...
    def forward_pass(self):
        ...
        for trans_layer in range(self.num_translayer):
            ...
            img_his_emb = self.ffn(self.multihead_attention(queries=tf.concat([img_emb, new_emb, loss_emb], 1),
                                                            ...
                                                            query_masks=tf.concat(
                                                                [imgmask, self.newmask, self.lossmask], 1),
                                                            key_masks=tf.concat([imgmask, self.newmask, self.lossmask],
                                                                                1)))  # (N, 2*length_his+1, d_model)
            img_emb = tf.expand_dims(img_his_emb[:, 0, :], 1)  # (N, 1, d_model)
            new_emb = img_his_emb[:, 1:self.length_his + 1, :]  # (N, length_his, d_model)
            loss_emb = img_his_emb[:, -self.length_his:, :]  # (N, length_his, d_model)
        ...
        logits_bert_new = tf.einsum('ijk,kl->ijl', tf.batch_gather(new_emb, self.newbertindex), self.dae_weights[-1]) + \
                          self.dae_biases[-1]  # (N, 3, num_softid)
        model_outputs.append(logits_bert_new)
        logits_bert_loss = tf.einsum('ijk,kl->ijl', tf.batch_gather(loss_emb, self.lossbertindex),
                                     self.dae_weights[-1]) + self.dae_biases[-1]  # (N, 3, num_softid)
        model_outputs.append(logits_bert_loss)
